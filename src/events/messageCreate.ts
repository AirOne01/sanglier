...
export default (client: Client, msg: Message) => {
  if (msg.author.tag === client.user.tag && msg.content === "🏓 Pong") {
    if (client.hasOwnProperty("lastDate")) {
      const timePassed: number = Date.now() - client["lastDate"];
  ...
  const cmd: { run: (client: Client, msg: Message, args: string[]) => void } =
    client["commands"].get(command);
  // gets the command from client.commands Enmap
  if (!cmd) {
    return msg.reply(
  ...
  try {
    ...
    console.log(e);
    msg.reply(
      "Une erreur est survenue pendant l'éxécution de cette commmande. dsl. ```\n" +
        e +
        "\n```"
    );
  }
